/**
 *
 * fn_parameters.cpp contains functions related to 
 * the parameters, which have been refactored 
 * out of the pindel.main 
 *
 * */
...
/* 
 * 'defineParameters' defines the parameters to be used by Pindel. 
 * Takes the variables from the calling function as argument for those variables which
...
void defineParameters( std::vector<Parameter *>& parameters ) {
    ...
    parameters.push_back( new StringParameter( &userSettings->referenceFilename, "-f", "--fasta", "the reference genome sequences in fasta format", true, "" ) );
    parameters.push_back( new StringParameter( &userSettings->pindelFilename, "-p", "--pindel-file", "the Pindel input file; either this, a pindel configuration file (consisting of multiple pindel filenames) or a bam configuration file is required", false, "" ) );
    parameters.push_back( new StringParameter( &userSettings->bamConfigFilename, "-i", "--config-file", "the bam config file; either this, a pindel input file, or a pindel config file is required. Per line: path and file name of bam, insert size and sample tag.  For example: /data/tumour.bam  400  tumour", false, "" ) );
    parameters.push_back( new StringParameter( &userSettings->pindelConfigFilename, "-P", "--pindel-config-file", "the pindel config file, containing the names of all Pindel files that need to be sampled; either this, a bam config file or a pindel input file is required. Per line: path and file name of pindel input. Example: /data/tumour.txt", false, "" ) );
    parameters.push_back( new StringParameter( &userSettings->outputFilename, "-o", "--output-prefix", "Output prefix", true, "" ) );
    parameters.push_back( new StringParameter( &userSettings->userDefinedRegion, "-c", "--chromosome", "Which chr/fragment. Pindel will process reads for one chromosome each time. ChrName must be the same as in reference sequence and in read file. '-c ALL' will make Pindel loop over all chromosomes. The search for indels and SVs can also be limited to a specific region; -c 20:10,000,000 will only look for indels and SVs after position 10,000,000 = [10M, end], -c 20:5,000,000-15,000,000 will report indels in the range between and including the bases at position 5,000,000 and 15,000,000 = [5M, 15M] (default ALL)", false, "ALL" ) );
    parameters.push_back( new BoolParameter( &userSettings->showHelp, "-h", "--help", "show the command line options of Pindel", false, false ) );
    parameters.push_back( new BoolParameter( &userSettings->SearchDiscordantReadPair, "-R", "--RP", "search for discordant read-pair to improve sensitivity (default true)", false, true ) );
    parameters.push_back( new IntParameter(&userSettings->numThreads, "-T", "--number_of_threads", "the number of threads Pindel will use (default 1)", false, 1 ) );
    std::stringstream ss; 
    ss << "the maximum size of structural variations to be detected; the higher this number, the greater the number of SVs reported, but the computational cost and memory requirements increase, as does the rate of false positives. 1=128, 2=512, 3=2,048, 4=8,092, 5=32,368, 6=129,472, 7=517,888, 8=2,071,552, 9=8,286,208. (maximum "<< g_MAX_RANGE_INDEX <<"; default 4)";
    parameters.push_back( new IntParameter( &userSettings->MaxRangeIndex, "-x", "--max_range_index", ss.str(), false, 4 ) );
    parameters.push_back( new FloatParameter( &userSettings->FLOAT_WINDOW_SIZE, "-w", "--window_size", "for saving RAM, divides the reference in bins of X million bases and only analyzes the reads that belong in the current bin, (default 5 (=5 million))", false, 5.0 ) );
    parameters.push_back( new FloatParameter(&userSettings->Seq_Error_Rate, "-e", "--sequencing_error_rate", "the expected fraction of sequencing errors (default 0.01)", false, 0.01 ) );
    parameters.push_back( new FloatParameter(&userSettings->sensitivity, "-E", "--sensitivity", "Pindel only reports reads if they can be fit around an event within a certain number of mismatches. If the fraction of sequencing errors is 0.01, (so we'd expect a total error rate of 0.011 since on average 1 in 1000 bases is a SNP) and pindel calls a deletion, but there are 4 mismatched bases in the new fit of the pindel read (100 bases) to the reference genome, Pindel would calculate that with an error rate of 0.01 (=0.011 including SNPs) the chance that there are 0, 1 or 2 mismatched bases in the reference genome is 90%. Setting -E to .90 (=90%) will therefore throw away all reads with 3 or more mismatches, even though that means that you throw away 1 in 10 valid reads. Increasing this parameter to say 0.99 will increase the sensitivity of pindel though you may get more false positives, decreasing the parameter ensures you only get very good matches but pindel may not find as many events. (default 0.95)", false, 0.95 ) );
    ...
    parameters.push_back( new StringParameter( &userSettings->breakdancerFilename, "-b", "--breakdancer", "Pindel is able to use calls from other SV methods such as BreakDancer to further increase sensitivity and specificity. BreakDancer result or calls from any methods must in the format: ChrA LocA stringA ChrB LocB stringB other", false, "" ) );
...
// prints all parameters available.
void printHelp( const std::vector<Parameter *>& parameters ) {
    *logStream << std::endl << "Program:   pindel (detection of indels and structural variations)" << std::endl;
    *logStream << Pindel_Version_str << std::endl;
    *logStream << "Contact:   Kai Ye <kye@genome.wustl.edu>" << std::endl << std::endl;
    *logStream << "Usage:     pindel -f <reference.fa> -p <pindel_input> [and/or -i bam_configuration_file] -c <chromosome_name> -o <prefix_for_output_file>"
               << std::endl 
               << std::endl;
